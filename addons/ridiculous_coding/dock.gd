...
@onready var shake_intensity_spin: SpinBox = $VBoxContainer/VBoxContainer/SpinBox
...
func connect_checkboxes():
	...
	shake_intensity_spin.value_changed.connect(func(changed):
		shake_intensity = changed
...
func save_checkbox_state():
	...
	stats.set_value("settings", "shake_intensity", shake_intensity)
	stats.save(STATS_FILE)
...
func load_checkbox_state():
	if stats.load(STATS_FILE) == OK:
		...
		shake_intensity_spin.value = stats.get_value("settings", "shake_intensity", 5.0)
	else:
		...
		shake_intensity_spin.value = shake_intensity
